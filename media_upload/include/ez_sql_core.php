	...
	class ezSQLcore
	{
		...
		*/
		...
		function get_var($query=null,$x=0,$y=0)
		{
			...
			_d("get_var %s,%s",$query,$this->last_result[$y]);
		...
		*/
		...
		function get_row($query=null,$output=OBJECT,$y=0)
		{
			...
			_d("get_row %s,%s",$query,$this->last_result[$y]);
		...
		*/
		...
		function get_col($query=null,$x=0)
		{
			...
			_d("get_col %s,%s",$query,$new_array);
			return $new_array;
		}
		...
		function get_results($query=null, $output = OBJECT)
		{
			...
			elseif ( $output == ARRAY_A || $output == ARRAY_N )
			{
				if ( $this->last_result )
				{
					...
					_d("get_results %s,%s",$query,$new_array);
					return $new_array;
				}
				else
				{
					_d("get_results %s,%s",$query,array());
					return array();
				}
			}
		}
